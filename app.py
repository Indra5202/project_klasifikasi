...
# === Load model ===
model = joblib.load("model_format_checker.pkl")
...
<div class='centered-logo-title'>
    <div>
       <h1 style="font-size:50px; color:#2c5d94; text-align:center;">SWISS GERMAN UNIVERSITY</h1>
        ...
        <p style='font-size:16px; color:gray;'>Upload one paper PDF and let the app automatically check format structure compliance.</p>
    </div>
</div>
""", unsafe_allow_html=True)
...
# === Format headings to detect ===
HEADINGS = [
    "introduction", "related work", "system overview",
    "methodology", "experiment", "discussion", "conclusion"
]

# Reset session state if new PDF is uploaded
if pdf_file:
    ...
    # Find title
    title = next((line.strip() for line in lines if line and line[0].isupper()), lines[0])

    # Find student author
    student_author_line = next((line.strip() for line in lines if any(char.isalpha() for char in line) and ("," in line or ";" in line)), "")
    ...
    for heading in HEADINGS:
        detected[heading.capitalize()] = int(heading.lower() in text.lower())

    # Prediksi format
    feature_vector = [[detected[h.capitalize()] for h in HEADINGS]]
    pred = model.predict(feature_vector)[0]
    detected["status"] = "✅ Compliant" if pred == 1 else "❌ Non-compliant"

    st.success("✅ Analysis complete. Ready for reviewer evaluation.")
    ...
    with st.expander(f"📄 Review: {detected['file_name']}"):
        ...
        english_ok, english_issue = radio_with_comment("Is the manuscript written in proper and sound English?", "english")
        format_ok, format_comment = radio_with_comment("Format follows author guideline?", "format")
        sota_ok = st.radio("Is the problem state-of-the-art?", ["Yes", "No"], index=None, key="review_sota")
        clarity_ok = st.radio("Is the problem clearly stated?", ["Yes", "No"], index=None, key="review_clarity")
        figures_ok, figures_comment = radio_with_comment("Do figures/tables support the goal/result?", "figures")
        conclusion_ok, conclusion_comment = radio_with_comment("Does the conclusion answer the problem?", "conclusion")
        references_ok, references_comment = radio_with_comment("Are references up-to-date?", "references")

        recommendations = st.text_area("Recommendations:", key="review_recommend")
        overall_eval = st.selectbox("Overall Evaluation", ["", "Reject", "Accept with revision", "Full acceptance"], key="review_eval")
...
if "review_all" in st.session_state and st.session_state.review_all:
    ...
    st.download_button("💾 Download All Review Summary as CSV", csv, "review_summary.csv", "text/csv")
